...
public class MovingRotorTest {
    ...
    @Test
    public void checkRotorNotchesWorking() {
        setRotor("I", NAVALA, "D");
        MovingRotor rotor2 = new MovingRotor("II",
                new Permutation(
                        "(FIXVYOMW) (CDKLHUP) (ESZ) "
                                + "(BJ) (GR) (NT) (A) (Q)", UPPER), "");
        rotor2.set(0);
        rotor.set(0);
        int[] intArray = new int[]{9, 3, 3, 12, 20};
        int[] intArray2 = new int[]{0, 9, 9, 17, 7};
        for (int i = 0; i < 5; i += 1) {
            rotor.advance();
            if (rotor.atNotch()) {
                rotor2.advance();
            }
            assertEquals("This should be right",
                    intArray[i], rotor.convertForward(i));
            assertEquals("Rotor2 should also be right",
                    intArray2[i], rotor2.convertForward(i));
        }
    }
