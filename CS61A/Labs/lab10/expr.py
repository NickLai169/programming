...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        # self.operator.eval([env[i.string] if type(i) == Name else i.value for i in self.operands])

        # return [i for i in env.values() if type(i) != PrimitiveFunction]
        # return self.operator.eval(env).apply([i for i in env.values() if type(i) != PrimitiveFunction])

        # return self.operator.eval(env).apply([i.value if type(i) == Literal else env[i.string].value for i in self.operands])
        # print([i for i in self.operands])
        n = 0
        for i in self.operands:
            if type(i) == CallExpr:
                self.operands[n] = self.operands[n].operator.eval(env).apply([Number(i.value) if type(i) == Literal else env[i.string] for i in self.operands[n].operands if type(i) == Literal or type(i) == Name])
            n += 1
        # print(self.operands)
        ...
        # for i in self.operands:
        #     if type (i) == Literal:
        #         lst_1.append(Number(i.value))
        #     elif type(i) == Name:
        #         lst_1.append(env[i.string])
        #     elif type(i) == Number:
        #
        return self.operator.eval(env).apply([env[i.string] if type(i) == Name else Number(i.value) for i in self.operands if type(i) == Literal or type(i) == Name or type(i) == Number])
        # return [Number(i.value) if type(i) == Literal else env[i.string] for i in self.operands]
    #marker 1
...
new_env = global_env.copy()
new_env.update({'a': Number(1), 'b': Number(2)})
add = CallExpr(Name('add'), [CallExpr(Name('mul'), [Literal(3), Literal(4)]), Name('b')])
add.eval(new_env)
print(add.eval(new_env))

# print(True in [type(i) == PrimitiveFunction for i in new_env.values()])
