...
def make_slow(action, bee):
    ...
    def new_action(colony):
        my_act = Bee.action
        # print('bee slow duration: ', bee.slow_duration)
        temp_colony_time = colony.time
        if colony.time % 2 == 0 or bee.slow_duration <= 0:
            my_act(bee, colony)

        if bee.slow_duration >0:
            bee.slow_duration -= 1
    return new_action
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    if effect == make_slow:
        if hasattr(bee, 'slow_duration'):
            bee.slow_duration += duration
        else:
            bee.slow_duration = duration
...
for _ in range(5):        # bee should only move on odd times
    bee.action(colony)
    print(bee.place, 'current colony time: ', colony.time)
    colony.time += 1
print(bee.place, 'current colony time: ', colony.time)
print('bee slow duration remainder: ', bee.slow_duration)
'tunnel_0_2'
