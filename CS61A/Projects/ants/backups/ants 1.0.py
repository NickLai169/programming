        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    # print('current can contain new')
                    self.ant.contain_ant(insect)
                    lolly_var = None
                elif insect.can_contain(self.ant):
                    # print('new can contain current')
                    lolly_var = None
                    insect.contain_ant(self.ant)
                    self.ant = insect
                else:
                    lolly_var = 'YOU DONE FUCKED UP BOI'
                    # print(lollyvar)
                assert lolly_var is None, 'Two ants in {0}'.format(self)
                # END Problem 9
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        # print('is there an ant? ', self.place.ant is not None)
        # print('am I being blocked? ', self.place.ant.blocks_path)
    ...
    def action(self, colony):
        ...
        # END EC
        if self.blocked():
            # print("i'm killing this fucker lmao")
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        how_many_places = -1
        def find_nearest_bee(place_1):
            nonlocal how_many_places
            how_many_places += 1
            #print(how_many_places)
            if place_1 == hive:
                return None
            elif self.min_range != None and how_many_places < self.min_range:
                #print('it went through')
                return find_nearest_bee(place_1.entrance)
            elif self.max_range != None and how_many_places > self.max_range:
                return None
            elif place_1.bees == []:
                return find_nearest_bee(place_1.entrance)
            #print('it came here')
            return random_or_none(place_1.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0 and self.place.bees != []:
            # print('opt. 1')
            the_bee = random_or_none(self.place.bees)
            # print('the bee is :', the_bee)
            self.eat_bee(the_bee)
        elif self.digesting > 0:
            # print('opt. 2')
            self.digesting -= 1
        # print('action printing: ', self.place.bees)
        # END Problem 6
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        its armor to 0."""
        # BEGIN Problem 11
        self.temp_add_insect = Place.add_insect
        self.temp_add_insect(self, insect)
        '''^Why does this work?'''
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        #marker
        if QueenAnt.the_true_queen == None:
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        self.temp_reduce_armor = Insect.reduce_armor
        if self is QueenAnt.the_true_queen:
            self.temp_reduce_armor(self, amount)
            if self.armor <= 0:
                bees_win()
        else:
            self.temp_reduce_armor(self, amount)
...
# dimensions = (2, 9)
# colony = AntColony(None, hive, ant_types(),
#         dry_layout, dimensions)
#
# # QueenAnt Placement
# queen = QueenAnt()
# impostor = QueenAnt()
# front_ant, back_ant = ThrowerAnt(), ThrowerAnt()
...
# print('imposter armor: ', impostor.armor)            # Impostors must die!
#
# print('tunnel 4 ant: ', tunnel[4].ant)
#
# print('back ant damage: ', back_ant.damage)          # Ants should not be buffed
#
# print('front ant damage: ', front_ant.damage)
#
