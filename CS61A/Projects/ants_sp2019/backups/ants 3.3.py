...
def apply_effect(effect, bee, duration):
    ...
    if effect == make_scare:
        if hasattr(bee, 'scare_duration'):
            bee.scare_duration += duration
        else:
            bee.scare_duration = duration
...
print(bee.place.name, 'current colony timer: ', colony.time, bee.im_scared)
'tunnel_0_4'
for _ in range(3): # slow bee three times
   slow.action(colony)
colony.time = 1
bee.action(colony) # scared, but also slowed thrice
print(bee.place.name, 'current colony timer: ', colony.time, bee.im_scared, bee.slow_until)
'tunnel_0_4'
colony.time = 2
