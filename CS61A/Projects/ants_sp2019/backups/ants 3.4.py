...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if hasattr(self, 'im_scared'):
            if self.im_scared:
                destination = destination.entrance
                if not destination.entrance.name == 'Hive':
                    destination = destination.entrance
                self.im_scared -= 1
        # END EC
        if self.blocked():
...
def make_slow(action, bee):
    ...
    def new_action(colony):
        my_act = Bee.action
        # print('bee slow duration: ', bee.slow_until)
        temp_colony_time = colony.time
        if colony.time % 2 == 0 or colony.time > bee.slow_until:
            my_act(bee, colony)
...
def make_scare(action, bee):
    ...
    def new_action(colony):
        my_act = Bee.action
        if bee.scare_duration <= 0:
            my_act(bee, colony)
        else:
            if not hasattr(bee, 'im_scared'):
                bee.im_scared = bee.scare_duration
            my_act(bee,colony)
...
def apply_effect(effect, bee, duration):
    ...
    if effect == make_slow:
        if hasattr(bee, 'slow_until'):
            bee.slow_until += duration
        else:
            bee.slow_until = colony.time + duration
    if effect == make_scare:
        bee.scare_duration = 2
...
print(bee.place.name, 'current colony timer: ', colony.time,' bee scare duration: ', bee.im_scared)
'tunnel_0_4'
for _ in range(3): # slow bee three times
   slow.action(colony)
colony.time = 1
bee.action(colony) # scared, but also slowed thrice
print(bee.place.name, 'current colony timer: ', colony.time, ' bee scare duration: ', bee.im_scared, ' bee slowed until round ', bee.slow_until)
'tunnel_0_4'
colony.time = 2
