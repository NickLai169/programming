...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if self.ant is QueenAnt.the_true_queen or self.ant.is_container:
                # print('i came here tho -_-', self.ant, self.ant.contained_ant,  self.ant.contained_ant is QueenAnt.the_true_queen)
                if self.ant is QueenAnt.the_true_queen:
                    return
                if (self.ant.contained_ant is QueenAnt.the_true_queen):
                    # print('i came here')
                    return
            # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        #marker 1
        # BEGIN EC
        if hasattr(self, 'im_scared') and self.im_scared > 0:
            destination = self.place.entrance
            self.im_scared -= 1
            #print(self.im_scared)
        # END EC
        if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = -1
        def find_nearest_bee(location):
        	nonlocal place
        	place += 1
        	if location == hive:
        		return None
        	elif self.min_range != None and place < self.min_range:
        		return find_nearest_bee(location.entrance)
        	elif self.max_range != None and place > self.max_range:
                #We tried changing setting min_range and max_range's defaults to arbitrary numbers
                #but for some odd reason it wouldn't work, so we've changed it back :P
        		return None
        	elif location.bees == []:
        		return find_nearest_bee(location.entrance)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for i in self.place.bees:
        	if i.armor - self.damage > 0:
        		copy_list.append(i)
        		i.armor -= self.damage
        self.place.bees = copy_list
        # END Problem 7
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        self.water_add_insect = Place.add_insect
        self.water_add_insect(self, insect)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.the_true_queen == 'Nope':
        	QueenAnt.the_true_queen = self
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self is not QueenAnt.the_true_queen:
        	self.reduce_armor(self.armor)

        else:

            self.throw_action = ThrowerAnt.action
            self.throw_action(self, colony)

            self.list_of_behind = []
            self.temp_ant_place = self.place.exit

            while self.temp_ant_place:
                self.list_of_behind.append(self.temp_ant_place.ant)
                self.temp_ant_place = self.temp_ant_place.exit

            self.list_of_behind = list(filter(lambda x:x !=None, self.list_of_behind))


            for i in self.list_of_behind:
                if not hasattr(i, 'have_i_been_doubled'):
                    #print('damage doubling 1', i.name)
                    i.damage = i.damage * 2
                    i.have_i_been_doubled = True

                if i.is_container and i.contained_ant != None and not hasattr(i.contained_ant, 'have_i_been_doubled'):
                    #print('damage doubling 2', i.name)
                    i.contained_ant.damage = i.contained_ant.damage * 2
                    i.contained_ant.have_i_been_doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        if self is QueenAnt.the_true_queen:
        	if self.armor <= 0:
        		bees_win()
        elif self is not QueenAnt.the_true_queen:
        	self.place.remove_insect(self)
        # END Problem 13
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    #print(action)
    def TR_make_slow(bee, colony):
        nonlocal action

        # print("ye i'm slowing")
        if colony.time % 2 == 0:
            # print('i made it here', colony.time)
            action(colony)
...
def make_scare(action, bee):
    ...
    #marker 2
    # BEGIN Problem EC
    def TR_make_scare(bee, colony):
        nonlocal action
        if not hasattr(bee, 'im_scared'):
            bee.im_scared = 2
        action(bee)

    return TR_make_scare
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    #print('hi', old_action)
    #print('bye', new_action)

    def inner_effect(colony):
        nonlocal bee, duration, new_action, old_action
        if duration != 0:
            new_action(bee, colony)
            duration -= 1
            #print(new_action, 'duration', duration)
        else:
            #print('action revert', old_action)
            bee.action = old_action
            bee.action(colony)

    bee.action = inner_effect
    # END Problem EC
...
# print(bee.place.name, 'colony time is', colony.time)
#
# 'tunnel_0_4'
