...
def find_closest(location, centroids):
    ...
    # current_distance = distance(closest_centroid, location)
    # for i in centroids:
    #     if distance(location, i) < current_distance:
    #         closest_centroid = i
    #         current_distance = distance(location, i)
...
def find_centroid(cluster):
    ...
    lat_sum = 0
    for i in cluster:
        long_sum += restaurant_location(i)[0]
        lat_sum += restaurant_location(i)[1]
    return  [long_sum/len(cluster), lat_sum/len(cluster)]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        restaurant_groups = group_by_centroid(restaurants, old_centroids)
        centroids = []
        for i in restaurant_groups:
...
def rate_all(user, restaurants, feature_fns):
    ...
    the_dict = {}
    # for i in reviewed:
    #     the_dict[restaurant_name(i)] = user_rating(user, restaurant_name(i))
    # for i in restaurants:
    #     if restaurant_name(i) not in the_dict:
    #         the_dict[restaurant_name(i)] = predictor(i)
    for rest in restaurants:
        rest_name = restaurant_name(rest)
        if rest in reviewed:
            the_dict[rest_name] = user_rating(user,rest_name)
        else:
            the_dict[rest_name] = predictor(rest)
