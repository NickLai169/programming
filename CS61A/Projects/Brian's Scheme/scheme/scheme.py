...
def eval_all(expressions, env):
    ...
    while curr_expr is not nil:
        current = scheme_eval(curr_expr.first, env)
        curr_expr = curr_expr.second
    return current
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError('Argument values do not match the number of formal parameters')
        return child_frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        name = target.first
        lambda_func = do_lambda_form(Pair(target.second, expressions.second), env)
