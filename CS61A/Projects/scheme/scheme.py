...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        n = scheme_eval(first, env)

        check_procedure(n)

        rest = rest.map(lambda x: scheme_eval(x, env))

        return scheme_apply(n, rest, env)
        # END PROBLEM 5
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('something something {0}'.format('pineapples'))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        # print(expressions.first, scheme_listp(expressions.first))
        if scheme_listp(expressions.first):
            env.bindings[expressions.first.first] = LambdaProcedure(expressions.first.second, expressions.second, env)
