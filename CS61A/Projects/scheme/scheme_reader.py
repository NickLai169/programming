...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        #         return nil
        #     else:
        #         return Pair(src.remove_front(), walker(src))
        #
        # return Pair('quote', walker(src))
        # # END PROBLEM 7
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # if val in quotes.keys():
            #     return Pair(quotes[val], read_tail(src))
            # return Pair(val, read_tail(src))
            Pair(scheme_read(src), read_tail(src))
            # END PROBLEM 2
    except EOFError:
        raise SyntaxError('unexpected end of file')
